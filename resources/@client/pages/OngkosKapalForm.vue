<template>
    <div class="p-6 bg-white rounded shadow">
      <h2 class="text-xl font-bold mb-4">{{ isEdit ? 'Edit' : 'Tambah' }} Ongkos Kapal</h2>
      <form @submit.prevent="submit">
        ...
        <div class="mt-4 border-t pt-4">
          ...
          <div
            ...
            :key="i"
            class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2 items-end"
          >
            <div>
  ...
  async function submit() {
    ...
    if (form.details.length === 0) {
      return Swal.fire('Peringatan', 'Minimal 1 detail OA wajib diisi', 'warning')
    }
  
    loading.value = true
    try {
      const url = isEdit ? `/api/ongkos-kapal/${route.params.id}` : '/api/ongkos-kapal'
      const method = isEdit ? 'put' : 'post'
      await axios[method](url, form)
      Swal.fire('Sukses', 'Data disimpan', 'success')
      router.push({ name: 'ongkos-kapal-list' })
    } catch (err: any) {
  ...
  onMounted(async () => {
    await fetchAll()
    if (isEdit) {
      const { data } = await axios.get(`/api/ongkos-kapal/${route.params.id}`)
      Object.assign(form, {
